...
def testDrawTiles():
    ...
    for i in range(TEST_SAMPLE_SIZE):
        ...
        gpu.drawTile8x8(rnd_pos_x, rnd_pos_y, randrange(0, MAX_TILES, 1))
...
def drawRamTileSet8x8():
    ...
    for countY in range(TILE_SET_W):
        for countX in range(TILE_SET_H):
            ...
            pos_x = (50 + (countX * TLE_8X8_SIZE))
            pos_y = (50 + (countY * TLE_8X8_SIZE))

            gpu.drawTile8x8(pos_x, pos_y, count)
            count += 1


# Fill whole screen by random tiles
def fillScreenByTiles():
    max_x_size = int(gpu.width() / TLE_8X8_SIZE)  # \__ calculate how much
    max_y_size = int(gpu.height() / TLE_8X8_SIZE)  # /   tiles in x and y axis
