...
def testDrawTiles():
    ...
    for i in range(TEST_SAMPLE_SIZE):
        ...
        gpu.drawTile16x16(rnd_pos_x, rnd_pos_y, randrange(0, MAX_TILES, 1))
...
def drawRamTileSet16x16():
    ...
    for countY in range(TILE_SHOW_W):
        for countX in range(TILE_SHOW_H):
            ...
            pos_x = (50 + (countX * TLE_16X16_SIZE))
            pos_y = (50 + (countY * TLE_16X16_SIZE))

            gpu.drawTile16x16(pos_x, pos_y, count)
            count += 1


# Fill whole screen by random tiles
def fillScreenByTiles():
    max_x_size = int(gpu.width() / TLE_16X16_SIZE)   # \__ calculate how much
    max_y_size = int(gpu.height() / TLE_16X16_SIZE)  # /   tiles in x and y axis
