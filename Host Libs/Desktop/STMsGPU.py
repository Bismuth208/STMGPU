...
# ------------------------------------------------------------------------ #
_T_SGPU_REGISRERS_VER = "sGPU registers: v0.0.6"
...
class sGPU(object):
    ...
    def __init__(self):
        ...
        self.__version__ = _T_SGPU_REGISRERS_VER
    ...
    def drawPixel(self, x, y, color):
        self.__sendCommand(self.__pack_BHHH(
                DRW_PIXEL, int(x), int(y), int(color)))
    ...
    def fillRect(self, x, y, w, h, color):
        self.__sendCommand(self.__pack_BHHHHH(
                FLL_RECT,
                int(x), int(y),
                int(w), int(h),
                int(color)))
    ...
    def drawFastVLine(self, x, y, h, color):
        self.__sendCommand(self.__pack_BHHHH(
                DRW_V_LINE, int(x), int(y), int(h), int(color)))

    def drawFastHLine(self, x, y, w, color):
        self.__sendCommand(self.__pack_BHHHH(
                DRW_H_LINE, int(x), int(y), int(w), int(color)))

    def drawCircle(self, x, y, r, color):
        self.__sendCommand(self.__pack_BHHHH(
                DRW_CIRCLE, int(x), int(y), int(r), int(color)))

    def fillCircle(self, x, y, r, color):
        self.__sendCommand(self.__pack_BHHHH(
                FLL_CIRCLE, int(x), int(y), int(r), int(color)))

    def drawTriangle(self, x0, y0, x1, y1, x2, y2, color):
        self.__sendCommand(pack(
                '<BHHHHHHH', DRW_TRINGLE,
                int(x0), int(y0),
                int(x1), int(y1),
                int(x2), int(y2),
                int(color)))
    ...
    def __write(self, value):
        ...
        for c in str(value).encode('utf-8'):
            self.__sendCommand(self.__pack_Bc(DRW_PRNT_C, c))

    def drawChar(self, x, y, c, charColor, bgColor, size):
        self.__sendCommand(pack(
                '<BHHHHBB', DRW_CHAR,
                int(x), int(y),
                int(charColor), int(bgColor),
                int(c), int(size)))
    ...
    def setTextColorBG(self, charColor, bgColor):
        self.__sendCommand(self.__pack_BHH(
                SET_TXT_CR_BG, int(charColor), int(bgColor)))
    ...
    #     self.__sendCommand(self.__pack_BB(DRW_PRNT, int(len(text))))
    #     self.__sendCommand(text)

    # i have no options instead of add 'g' at end...
    def printg(self, value = None, intBase = None):
        ...
        if type(value) is int:
            text = b''
            if intBase is not None:
                text = self.__formaterBase[intBase] % (value)
            else:
                text = str(value)
            self.__write(text)

    def println(self, value = None, intBase = None):
        if type(value) is not None:
            self.printg(value, intBase)
        self.__write('\n')

    # --------- #
    def printAt(self, x, y, value, intBase = None):
        self.setCursor(x, y)
        self.printg(value, intBase)
    ...
    def setDispBrightness(self, brightness):
        self.__sendCommand(self.__pack_BB(
                SET_BRIGHTNES, int(brightness)))
    ...
    def __sendTileData(self, tileType, posX, posY, tileNum):
        self.__sendCommand(pack(
                '<BHHB', int(tileType), 
                int(posX), int(posY), int(tileNum)))

    def __loadTileBase(
            self, tileType, tileSetArrName, tileSetW,
            ramTileBase, tileMin, tileMax =0, isTileset = False):
        if len(tileSetArrName) > 0:
            cmdBuffer = b''
            if isTileset:
                cmdBuffer = pack(
                        '<BBBBBB', int(tileType), int(len(tileSetArrName)),
                        int(tileSetW), int(ramTileBase),
                        int(tileMin), int(tileMax))
            else:
                # in this case 'tileMin' works as 'tileNum'
                cmdBuffer = pack(
                        '<BBBBB', int(tileType), int(len(tileSetArrName)),
                        int(tileSetW), int(ramTileBase), int(tileMin))
    ...
    # ---- tile 8x8 ---- #
    def loadTile8x8(
            self, tileSetArrName, tileSetW, ramTileNum, tileNum):
        self.__loadTileBase(
                LDD_TLE_8, tileSetArrName, tileSetW,
                ramTileNum, tileNum)

    def loadTileSet8x8(
            self, tileSetArrName, tileSetW, ramTileBase,
            tileMin, tileMax):
        self.__loadTileBase(
                LDD_TLES_8, tileSetArrName, tileSetW,
                ramTileBase, tileMin, tileMax, True)

    def drawTile8x8(self, posX, posY, tileNum):
        self.__sendTileData(DRW_TLE_8, posX, posY, tileNum)

    # ---- tile 16x16 ---- #
    def loadTile16x16(
            self, tileSetArrName, tileSetW, ramTileNum, tileNum):
        self.__loadTileBase(
                LDD_TLE_16, tileSetArrName, tileSetW,
                ramTileNum, tileNum)
    ...
    def loadTileSet16x16(
            ...
            tileMin, tileMax):
        self.__loadTileBase(
                LDD_TLES_16, tileSetArrName, tileSetW,
                ramTileBase, tileMin, tileMax, True)

    def drawTile8x8(self, posX, posY, tileNum):
        self.__sendTileData(DRW_TLE_16, posX, posY, tileNum)

    # ---- tile 32x32 ---- #
    def loadTile32x32(
            self, tileSetArrName, tileSetW, ramTileNum, tileNum):
        self.__loadTileBase(
                LDD_TLE_32, tileSetArrName, tileSetW,
                ramTileNum, tileNum)
    ...
    def loadTileSet32x32(
            ...
            tileMin, tileMax):
        self.__loadTileBase(
                LDD_TLES_32, tileSetArrName, tileSetW,
                ramTileBase, tileMin, tileMax, True)

    def drawTile8x8(self, posX, posY, tileNum):
        self.__sendTileData(DRW_TLE_32, posX, posY, tileNum)

    # universal tile draw 8, 16, 32
    # def drawTiile(self, posX, posY, tileType, tileNum):
    #     self.__sendCommand(pack(
    #             '<BHHBB', DRW_TLE_U, int(posX), int(posY),
    #             int(tileType), int(tileNum)))

    def loadTileMap(self, tileMapFileName = None):
        if type(tileMapFileName) is str:
            self.__sendCommand(self.__pack_BB(
                    LDD_TLE_MAP, int(len(tileMapFileName))))
    ...
    def setSpritePosition(self, sprNum, posX, posY):
       # is it an error? maybe = BBHH ?
        self.__sendCommand(self.__pack_BHHH(
                SET_SPR_POS, int(sprNum), int(posX), int(posY)))

    def setSpriteType(self, sprNum, sprType):
        self.__sendCommand(pack(
                '<BBB', SET_SPR_TYPE, int(sprNum), int(sprType)))

    def setSpriteVisible(self, sprNum, state):
        self.__sendCommand(pack(
                '<BBB', SET_SPR_VISBL, int(sprNum), int(state)))

    def setSpriteTiles(self, sprNum, tle1, tle2, tle3, tle4):
        self.__sendCommand(pack(
                '<BBBBBB', SET_SPR_TLE, int(sprNum),
    ...
    def getSpriteCollision(self, sprNum1, sprNum2):
        self.__sendCommand(pack(
                '<BBB', GET_SRP_COLISN, int(sprNum1), int(sprNum2)))
    ...
    # ----------------- SD card ---------------- #
    def loadPalette(self, palleteArrName = None):
        if type(palleteArrName) is str:
            self.__sendCommand(self.__pack_BB(
                    LDD_USR_PA, int(len(palleteArrName))))
            self.__sendCommand(palleteArrName)

    def printBMP(self, posX =0, posY =0, bmpFileName = None):
        if bmpFileName is str:
            self.__sendCommand(pack(
                  '<BHHB', DRW_BMP_FIL, 
                  int(posX), int(posY), int(len(bmpFileName))))
            self.__sendCommand(bmpFileName)

    # ------------------ Sound ----------------- #
    def playNote(self, freq, duration):
        self.__sendCommand(self.__pack_BHH(
                SND_PLAY_TONE, int(freq), int(duration)))
    ...
    def setTextColorGUI(self, textColor, bgColor):
        self.__sendCommand(self.__pack_BHH(
                SET_WND_CR_TXT, int(textColor), int(bgColor)))
    ...
    def setColorWindowGUI(self, frame, border):
        self.__sendCommand(self.__pack_BHH(
                SET_WND_CR, int(frame), int(border)))

    def drawWindowGUI(self, posX, posY, w, h, text = None):
        cmdBuffer = b''
        if type(text) is str:
            cmdBuffer = pack(
                    '<BHHHHB', DRW_WND_TXT,
            ...
            cmdBuffer = self.__pack_BHHHH(
                    DRW_WND_AT, int(posX), int(posY), int(w), int(h))
        self.__sendCommand(cmdBuffer)
    ...
    def setCamPosition(self, posX, posY, angle):
        self.__sendCommand(self.__pack_BHHH(
                SET_CAM_POS, int(posX), int(posY), int(angle)))
    ...
    def setSkyFloor(self, sky, floor):
        self.__sendCommand(self.__pack_BHH(
                SET_BACKGRND, int(sky), int(floor)))
