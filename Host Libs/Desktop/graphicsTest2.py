...
def testFillScreen():
    ...
    for color in [gpu.COLOR_BLACK, gpu.COLOR_RED,
                  gpu.COLOR_GREEN, gpu.COLOR_BLUE,
                  gpu.COLOR_BLACK]:
...
def testText():
    ...
    gpu.setTextSize(3)
    gpu.println(0xDEADBEEF, gpu.HEX)
...
def testHLinesHelp(x0, y0, y1):
    for x1 in range(0, gpu.width(), 6):
        gpu.drawLine(x0, y0, x1, y1, gpu.COLOR_CYAN)
...
def testFastLines():
    ...
    for y in range(0, h, 5):
        gpu.drawFastHLine(0, y, w, gpu.COLOR_RED)

    for x in range(0, w, 5):
        gpu.drawFastVLine(x, 0, h, gpu.COLOR_BLUE)
    gpu.pingCommand()
    return time.time() - t
...
def testRects():
    ...
    for i in range(2, n, 6):
        i2 = int(i / 2)
        gpu.drawRect(cx - i2, cy - i2, i, i, gpu.COLOR_GREEN)
    gpu.pingCommand()
    return time.time() - t
...
def testFilledRects():
    ...
    for i in range(n, 0, -6):
        i2 = int(i / 2)
        gpu.fillRect(cx - i2, cy - i2, i, i, gpu.COLOR_YELLOW)
        gpu.drawRect(cx - i2, cy - i2, i, i, gpu.COLOR_MAGENTA)
    gpu.pingCommand()
    return time.time() - t
...
def testCircles():
    ...
    for x in range(0, w, r2):
        for y in range(0, h, r2):
            gpu.drawCircle(x, y, radius, gpu.COLOR_WHITE)
    gpu.pingCommand()
    return time.time() - t
...
def testFilledCircles():
    ...
    for x in range(radius, w, r2):
        for y in range(radius, h, r2):
            gpu.fillCircle(x, y, radius, gpu.COLOR_MAGENTA)
    gpu.pingCommand()
    return time.time() - t
...
def main():
    ...
    while 1:
        ...
        for func in test_functions:
            print("%s: %d" % (func, test_functions[func]() * 1000))
            # time.sleep(.5)
        print('Done!')
